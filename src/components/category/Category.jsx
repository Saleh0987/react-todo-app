...
const Category = () => {
  ...
  useEffect(() => {
    const personalCategories = todosItem.filter((item) => item.category === 'personal');
    setPersonalTodos(personalCategories);

    const businessCategories = todosItem.filter((item) => item.category === 'business');
    setBusinessTodos(businessCategories);

    const othersCategories = todosItem.filter((item) => item.category === 'others');
  ...
  return (
    <div className='w-full lgl:w-[850px] h-[200px] md:h-[150px] bg-bodyColor flex flex-col md:flex-row items-center justify-start'>
      ...
      <div className='w-5/6 h-full flex items-center overflow-y-scroll scrollbar-hide'>
        <p className={`${activePersonal || activeBusiness || activeOthers ? 'hidden' : 
        ...
        {activePersonal && (
          <ul className="w-full h-[90%] flex flex-col items-center gap-2 px-2">
          {
            ...
            >Personal todo list is empty!</motion.li>
          }
        </ul>
        )}
        ...
        {activeBusiness && (
          <ul className="w-full h-[90%] flex flex-col items-center gap-2 px-2">
          {
            ...
            transition={{ duration: 0.5 }}
            className='w-full h-full text-lg flex justify-center items-center text-red-500 text-center font-semibold'
            >Business todo list is empty!</motion.li>
          }
        </ul>
        )}
        ...
        {activeOthers && (
          <ul className="w-full h-[90%] flex flex-col items-center gap-2 px-2">
          {
            ...
            transition={{ duration: 0.5 }}
            className='w-full h-full text-lg flex justify-center items-center text-red-500 text-center font-semibold'
            >Others todo list is empty!</motion.li>
          }
        </ul>
        )}
